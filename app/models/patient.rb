class Patient < ActiveRecord::Base
  ...
  def self.maximum_age(age = nil)
    if age
      ...
      where(['birthday >= ? OR birthyear >= ?', maximum_birthday, maximum_birthyear])
    else
      all
    end
  end
  ...
  def self.minimum_age(age = nil)
    if age
      ...
      where(['birthday <= ? OR birthyear <= ?', minimum_age, minimum_birthyear])
    else
      all
    end
  end
  ...
  def gender
    self[:gender].try(:capitalize)
  end

  def birthday_unknown
    @birthday_unknown != '0' && @birthday_unknown != 0 && @birthday_unknown
  end
