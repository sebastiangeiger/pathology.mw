def click_in_top_bar(*click_path)
  fail 'More than two levels are currently not supported' unless click_path.size <= 2
...
When(/^I go to the patient page for "(.*?)"$/) do |patient_name|
  ...
  step %(go to the patients overview page)
  row = all('tr').select do |tr|
    tr.text.include?(first_name) &&
    tr.text.include?(last_name)
  end.first
  if row && target = row['data-link-to']
    visit target
  else
